...
func evalSET(args []string) []byte {
	...
	// putting the key and value in the hashtable
...
func evalGET(args []string) []byte {
	...
	// if key is already expired then return nil
	if hasExpired(obj) {
		return RESP_NIL
	}
...
func evalTTL(args []string) []byte {
	...
	// if object does not exist, return RESP encoded -2 denoting key does not exist
	if obj == nil {
		return RESP_MINUS_2
	}

	// if obj exist but no expiration time is set on it then send -1
...
func EvalAndRespond(cmds RedisCmds, c *Client) {
	...
	for _, cmd := range cmds {
		// if txn is not in progress, then simply execute the command and add to the buffer
		if !c.isTxn {
			executeCommandToBuffer(cmd, buf, c)
			continue
		}

		// if txn is in progress, we enqueue the command and add QUEUED response to the buffer
		if !txnCommands[cmd.Cmd] {
			c.TxnQueue(cmd)
			buf.Write(RESP_QUEUED)
		} else {
			// if txn is active and command is non queable i.e. EXEC, DISCARD
